...
public final class ElwhaTabsIconsSmoke {
  ...
  private static void checkFormSizing() {
    ...
    check(
        "icon-only preferred width is 24 + padding",
  ...
  private static void checkBarHeightAndAlignment() {
    ...
    check(
        "bar preferred height grows to 64 with a stacked tab", bar.getPreferredSize().height == 64);

    bar.setSize(BAR_WIDTH, 64);
    bar.doLayout();
    check(
        "inline tab bottom-aligns in the 64px bar",
        labelOnly.getY() == 16 && labelOnly.getHeight() == 48);
    check(
        "stacked tab fills the 64px bar",
        bar.getTabAt(0).getY() == 0 && bar.getTabAt(0).getHeight() == 64);
  ...
  private static void checkIndicatorSpans() {
    ...
    final int labelW =
        stacked
            .getFontMetrics(com.owspfm.elwha.theme.TypeRole.TITLE_SMALL.resolve())
            .stringWidth("Home");
    ...
    final int infoW =
        inline
            .getFontMetrics(com.owspfm.elwha.theme.TypeRole.TITLE_SMALL.resolve())
            .stringWidth("Information");
    check(
        "inline span is icon + gap + label",
    ...
    check(
        "indicator hugs the icon-only span",
  ...
  private static void checkIconPaint() {
    ...
    check(
        "active icon area carries PRIMARY pixels",
        regionHasColorNear(
            image,
            active.getX() + icon.x,
            icon.y,
    ...
    check(
        "inactive icon area carries ON_SURFACE_VARIANT pixels",
        regionHasColorNear(
            image,
            inactive.getX() + icon2.x,
            icon2.y,
            icon2.width,
            icon2.height,
