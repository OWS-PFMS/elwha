...
`ShadowPainter` is a new static helper at `com.owspfm.elwha.theme.ShadowPainter`, parallel to `SurfacePainter` and `RipplePainter`. It extracts the round-rect blur logic from `SurfacePainter.renderShadowImage`, upgrades it to a **key + ambient two-shadow stack** matching M3 elevation tokens, and redesigns the cache:

- **Two-shadow stack (per M3 elevation tokens):** each paint composites a **key** pass (sharp, narrow, small downward offset) over an **ambient** pass (wider, softer, larger downward offset). Per-pass blur radius / offsetY / opacity are taken from M3's elevation-token table — key `{2,2,3,3,4}` blur, `{1,1,1,2,4}` offsetY across levels 1–5; ambient `{3,6,8,10,12}` blur, `{1,2,4,6,8}` offsetY. The larger ambient offsetY is what produces the visible directional drop (more shadow below, narrow rim above). Source alphas are held slightly above M3's stated opacities (key 160, ambient 90) to compensate for box-blur's lower per-pixel density vs CSS Gaussian. *This was originally deferred as a v2 enhancement; surfaced during smoketest that the single-Gaussian look diverged visibly from M3, so promoted to v1.*
