# Elwha Design Direction — Design-System Stance

**Status:** Working direction, not yet locked. Captures a philosophical design conversation about what Elwha should *be* before more components ship.
...
**Related:**
- [`elwha-extraction-decisions.md`](elwha-extraction-decisions.md) — strategic decisions made at extraction time.
- [`elwha-coupling-audit.md`](elwha-coupling-audit.md) — the audit that confirmed extraction-readiness.
- Open epics: [#27](https://github.com/OWS-PFMS/elwha/issues/27) (`FlatPill` → `ElwhaChip` — complete in lib), OWS-Local-Search-GUI#252 (`ElwhaList<T>` selection/drag unification), OWS-Local-Search-GUI#253 (`ElwhaCard` V2 API).
...
1. **Elwha is a design system, not a component grab-bag.** The pages-feel-non-cohesive smell in OWS-Local-Search-GUI is the forcing function. Components are downstream of the design language.
...
3. **FlatLaf is the substrate, not the design system.** Elwha configures FlatLaf via `UIManager` keys and ships the components Swing doesn't have (Chip, Card, their list containers). FlatLaf paints `JButton` et al.; Elwha tokens give them a coherent look.
...
6. **Tokens ship inside Elwha as a single artifact.** No separate tokens repo. Consumer palettes (e.g. OWS's brand colors) live in the consumer codebase; Elwha owns the vocabulary, not the values.
7. **Swing stays the substrate.** JavaFX is not a serious alternative for the OWS context (see footnote).

---

## 1. Framing — what problem is Elwha solving?
...
Implication: Elwha's primary forcing function is OWS's real needs. The API must not encode OWS-specific assumptions (coupling stance already enforces this), but it also must not pre-build for hypothetical consumers.
...
Elwha should lift Material 3's token vocabulary almost wholesale.
...
- The user already likes the Material look. Confirmed: Elwha aims to be Material-*flavored*, in the same way MUI is now Material-flavored rather than spec-compliant. Drift from spec is expected and acceptable.

**Not the same as adopting Material Design as a brand.** Elwha does not claim spec compliance; it does not need to. It claims to use the M3 *token taxonomy* as its semantic API.
...
**One source of truth: `UIManager`.** Every token lives there under a `Elwha.*` namespace (`Elwha.color.primary`, `Elwha.shape.md`, `Elwha.type.bodyMedium`, etc.).

**A typed facade on top.** Enums (or records) that know how to resolve themselves from `UIManager`. Components never call `UIManager.getColor("Elwha.color.primary")` directly — they go through the facade.
...
2. The typed facade gives Elwha components type safety, IDE autocomplete, and refactor-safety.
...
public enum ColorRole {
  ...
  public Color resolve() {
    return UIManager.getColor("Elwha.color." + key());
  }
}
...
```java
ElwhaTheme.builder(FlatLightLaf.class)
    .palette(MaterialPalettes.indigo())
    .typography(Typography.system())
    .install();
```

The builder writes both `Elwha.*` keys *and* the FlatLaf-native keys (`Button.background`, `Component.arc`, etc.) so raw Swing inherits the design language.
...
- **Most of the V2 `ElwhaCard` escape-hatches dissolve.** `setSurfaceColor` becomes "pick a different variant, or override the *role* at theme level." `setKeepSummaryWhenExpanded` was never a color thing — stays.
...
1. **Compute and bake.** At theme install, alpha-blend the state-layer opacity over the role color and write the *result* into FlatLaf's `*hoverBackground` / `*pressedBackground` keys. Math runs once per install. Components painted by FlatLaf look right. Elwha's own components (ElwhaChip, ElwhaCard) paint the overlay live at runtime since they own their paint.
...
| **Chip** | Doesn't exist in Swing → build (have `ElwhaChip`). |
| **Card** | Doesn't exist in Swing → build (have `ElwhaCard`). |
...
Direction: **make Elwha components `final`** (or otherwise sealed against public extension), and lean on MUI-style composition primitives:

- **Variants** (already present) — small enum of pre-baked configurations.
- **Slots** — named injection points for sub-content (leading icon, trailing icon, label, delete affordance, header, footer). The `ElwhaCard.setHeader` overload pattern in V1 is a slot API in disguise; making this explicit cleans up much of the V2 epic (#253).
- **Theming** — centralized token resolution via `ElwhaTheme` / `UIManager`.
- **Interaction modes** (already present on cards).

The V2 `ElwhaCard` API escape-hatches (`setSurfaceColor`, `setKeepSummaryWhenExpanded`, raw label getters, `setHeader` overloads) are the *symptom* of an effectively-final-ish component with no coherent composition story. Each new consumer need became a new setter. Explicit slot + variant + theme model is the cure.
...
- **Sealed tokens.** Consumers must go through `ElwhaChipVariant`, `ElwhaCardVariant`, etc. Cohesion enforced by *structural* constraint, not by discipline. **Risk:** every new consumer need becomes a new variant, recreating the V2-API escape-hatch problem at the variant level.
...
## 12. FlatLaf is the substrate, Elwha is the design system

Mental model — four layers, each owning one job:

```
┌────────────────────────────────────────────────┐
│ Elwha components (ElwhaChip, ElwhaCard, ...) │  ← we build, read tokens
├────────────────────────────────────────────────┤
│ Elwha tokens / theme (color roles, shape,   │  ← we build, the design language
...
- **Elwha : FlatLaf :: a design-system config : Tailwind.** Not replacing Tailwind; configuring it semantically and adding the bespoke components Tailwind doesn't ship.
...
**Tokens ship inside Elwha.** OWS-Local-Search-GUI depends on Elwha; that pulls components *and* tokens together. No separate `elwha-tokens` repo.
...
- **Elwha's tokens are not generic design tokens.** Adobe Spectrum / Salesforce Lightning / GitHub Primer publish token repos because they target web + iOS + Android + Figma simultaneously and need one source of truth across substrates. Elwha's tokens are tightly bound to Swing + FlatLaf's `UIManager`. No cross-substrate pressure justifying the split.
- **One consumer today.** Extract when duplication is *real* and *painful*, not for hypothetical reuse — same rule that justified extracting Elwha from OWS-tool in the first place.

**Where OWS's own brand palette lives:** in OWS-Local-Search-GUI, not in Elwha.

A palette (concrete colors mapped to roles) is just *data* — a `Map<ColorRole, Color>` or a small builder. OWS defines `OwsPalette.light()` / `OwsPalette.dark()` in its own codebase and passes it to `ElwhaTheme.builder(...).palette(owsPalette).install()`. **The token vocabulary is Elwha's; the palette values are the consumer's.** No third repo.

**Cheap future-proofing — enforce a clean internal package boundary:**

```
com.owspfm.elwha.theme.*     ← tokens, palette interface, theme installer
com.owspfm.elwha.card.*      ← uses theme; never the other way
com.owspfm.elwha.chip.*      ← uses theme; never the other way
com.owspfm.elwha.list.*  ← uses theme; never the other way
```
...
- Multiple sibling Swing component libs (e.g. `elwha-charts`, `elwha-forms`) start sharing tokens.
...
## 14. Adjacent ecosystem — the gap Elwha fills
...
1. ~~Lock the **token taxonomy**~~ — **DONE (2026-05-14).** See [`elwha-token-taxonomy.md`](elwha-token-taxonomy.md), now a locked-decisions doc: 46 tokens across color / type / shape / spacing / state.
2. ~~Define the **`ElwhaTheme` install API**~~ — **DONE (2026-05-14).** See [`elwha-theme-install-api.md`](elwha-theme-install-api.md), now a locked-decisions doc.
3. ~~Build **one reference palette** (light + dark)~~ — **DONE (2026-05-14).** `MaterialPalettes.baseline()` ships the M3 baseline scheme (light + dark) as a JSON resource loaded by `PaletteLoader`. Steps 1–3 are now implemented in code under `com.owspfm.elwha.theme` (Epic #30): the five token-facade enums, the `ElwhaTheme` install API, the FlatLaf-native key mapping, and the baseline palette + loader.
4. ~~Retrofit `ElwhaChip` variants~~ — **plan LOCKED (2026-05-14):** see [`elwha-flatchip-rebuild.md`](elwha-flatchip-rebuild.md). Grew from a narrow retrofit into a token-native rebuild of ElwhaChip's styling API (treatment-only variants, M3 types as factory presets, typed role/scale setters, `WARM_ACCENT` removed, auto-contrast deleted). Implementation waits on steps 1–3 landing in code. Serves as the pattern for `ElwhaCard`.
5. Use the token layer as the foundation of the **`ElwhaCard` V2 API** (#253), retiring the accumulated escape-hatches.
6. Revisit **`ElwhaList<T>` selection/drag unification** (#252) once selection state has a token-based visual story.
...
- **Migration cost for OWS-Local-Search-GUI is enormous** — and that, not Elwha, is the actual decision point. Elwha is the dependent. A consumer migration to JavaFX would require a Elwha rewrite, not a port.
