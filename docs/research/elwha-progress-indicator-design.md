...
- **Circular indeterminate**: over a 6000ms cycle the sweep grows/shrinks (sweep oscillates ~[20°, 280°] via `STANDARD`-eased grow/shrink phases) while base rotation advances 1080°/cycle plus a 360° advance kick every 1500ms over 300ms. **Implementation note:** this is a faithful interpretation of the Compose constants (R§C) — tuned visually at smoke; record adjustments here.
  - *S4 build decisions (2026-06-11):* the sweep follows a `STANDARD`-eased head/tail pair over the 6000ms cycle (head in the first half, tail in the second), scaled by a 0.78-turn cap (~280° peak) with a 6° visible-dot floor; the tail's own travel feeds the start angle, so grow and shrink both advance the head clockwise. **No track paints in indeterminate** — matches the classic M3 circular indeterminate and the spec renders (the visible-track anatomy is determinate-only); the linear indeterminate keeps its track spans per R§C.
