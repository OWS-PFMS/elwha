# Elwha Extraction — Locked Decisions

**Deliverable for:** [issue #259](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/259) — strategic decisions for the Elwha library extraction.
...
|---|---|---|
| 1 | Repo strategy | **Separate GitHub repo: `OWS-PFMS/elwha`** |
| 2 | Library name | **`elwha`** |
...
**Locked:** Create a new repo at `OWS-PFMS/elwha`.
...
**Why:** Short, descriptive, matches the internal vocabulary already used in this codebase ("Elwha components"). Brandable enough to be the canonical name in `build.gradle` snippets.
...
**Locked:** GitHub Packages (`maven.pkg.github.com/OWS-PFMS/elwha`).
...
**Why:** Semantic versioning convention. `0.x.y` indicates "API not yet stable; breaking changes are permitted between minor versions." This is honest given that two breaking-change epics are queued post-extraction: [#251](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/251) (FlatPill → ElwhaChip rename) and [#252](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/252) (ElwhaList<T> selection-surface extension).

**Path to 1.0.0:** Bump to `1.0.0` only after (a) the rename is complete, (b) the selection surface is stabilized, (c) [#253](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/253) (ElwhaCard V2) is complete. Until then, expect `0.2.x`, `0.3.x`, etc. as those epics land.
...
**Locked:**
- New repo `OWS-PFMS/elwha` gets its **own GitHub Issues**.
...
- Label set on the new repo mirrors `OWS-Local-Search-GUI`'s relevant labels: `technical-debt`, `developer-experience`, `enhancement`, `bug`, plus any Elwha-specific labels that emerge during work.
...
- **Coupling audit** (sub-issue [#260](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/260)): `docs/research/elwha-coupling-audit.md` — found zero coupling sites; library is extraction-ready as-is. Sub-issue [#261](https://github.com/OWS-PFMS/OWS-Local-Search-GUI/issues/261) (break couplings) closed as **not required**.
