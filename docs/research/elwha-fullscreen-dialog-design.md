...
> **Update ([#291](https://github.com/OWS-PFMS/elwha/issues/291)) — the 560 cap is no longer hard-locked.** Smoke-testing in a wide window showed the fixed 560 column reading as a narrow strip in a large empty surface (which invites "why not a Basic dialog?"). The column max width is now configurable via `Builder.contentMaxWidth(int)` (default **560**, the readable form width; `Integer.MAX_VALUE` = fill the frame width minus padding, for wide content like tables/dashboards). The default and the centering rule are unchanged — forms still shouldn't sprawl (over-wide line length is exactly why M3 caps content) — so this un-locks the call without changing the out-of-the-box look. The "prefer a Basic dialog on expanded widths; reach for full-screen on compact widths or for long, full-height input flows" guidance stands.
...
### §11.2 Action-button ripple suppressed during the exit fade [resolved in #288]

The exit fade composites a **one-time snapshot** of the surface at uniform alpha (a live subtree can't be faded — each child resets the alpha composite). A press ripple on an action button (close / confirm) clicked at dismiss would therefore be captured into that snapshot and **freeze mid-stroke** as the dialog fades out, instead of completing. This is inherent to the snapshot-based fade and is **not** fixable by keeping the ripple live (breaks the fade) or by timing tricks (a delayed close feels laggy). **Resolved in epic [#288](https://github.com/OWS-PFMS/elwha/issues/288):** `ElwhaButton` / `ElwhaIconButton` gained `setRippleEnabled(boolean)`, and the dialog disables the press ripple on its dismiss action buttons — the dialog-owned close ✕ and the consumer `confirmAction` — since the exit motion is itself the dismiss feedback ("no ripple > frozen ripple"). #288 applied the same suppression to the Basic Dialog's action row.
