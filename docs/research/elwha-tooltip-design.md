...
Anchor bounds convert to pane coordinates via `SwingUtilities.convertRectangle`; `layoutSurface` re-derives placement on host resize. A **`ShadowBearing` anchor's halo reserve is backed out** of the anchor rect before placing — the shadow band is not visual anchor, and leaving it in reads as a too-wide gap (added at smoke; pinned in `TooltipPlainChromeSmoke`). **Residual closed in [#493](https://github.com/OWS-PFMS/elwha/issues/493):** placement used to measure from component bounds, so a component stretched by a fill layout anchored at its stretched edge rather than its centered body. The public accessor that was missing now exists — `BodyBearing.bodyBoundsOf` (conventions §8b) — and it supplies both corrections, halo and stretch, from one call, so the hand-rolled halo back-out described above is gone. The surface's preferred size is content-driven (wrap math), never `getMaximumSize`-overridden (#199/#200 doctrine).
