...
10. **Badges day-one:** `ElwhaTab implements IconBearing`; `setBadge(ElwhaBadge)` follows the M3 badge-spec placement rows — `ICON_CORNER` while the icon is the visual anchor (stacked / icon-only), `LABEL_TRAILING` (new anchor mode: inline right after the label text, 4px gap, vertically centered — the spec's "Label 999+" row) once a label sits beside the content (inline, secondary, label-only), re-pinned on form changes. ⚠️ Corrected twice at smoke: "has icon ⇒ icon corner" flattened inline labels (round 1); the rail's cell-edge `TRAILING_EDGE` strands the badge in a wide fixed cell whose content is centered (round 2 — the rail never hits this because its rows hug content). §3.
...
- `setBadge(ElwhaBadge)` / `getBadge()` — anchors via `ElwhaBadgeAnchor` per the M3 badge-spec placement rows: `ICON_CORNER` while the icon is the visual anchor (stacked / icon-only); `LABEL_TRAILING` once a label sits beside the content (inline primary, all secondary icon tabs, label-only) — the badge flows inline right after the label text with a 4px gap, vertically centered (icon-corner there flattens the adjacent label; the rail's cell-edge `TRAILING_EDGE` strands it in a wide fixed cell — both caught at smoke). Re-pins automatically on `setInlineIcon`/variant restamps. `implements IconBearing.getIconBounds()` is the anchor feed: the icon rect for corner forms, the label rect for the trailing form.
