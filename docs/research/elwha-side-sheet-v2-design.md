...
**Standard — scrub the open/close width (refined during build).** No scrim, no host. Rather than translating the body (which leaves a gap in a fixed-width layout slot — a standard sheet can't slide "off"), the standard drag scrubs the **open-width animation**: dragging toward the edge narrows the sheet 1:1 with the pointer (`openProgress = 1 − fraction`), which *is* M3's coplanar squash, and siblings reflow as they would during a normal `close()`. Release ≥ threshold continues `openAnimator.reverse()` to width 0 (`close()`); release < threshold runs `openAnimator.start()` back to full width. This mirrors the modal path exactly — both scrub an existing animation rather than introduce a parallel drag transform — and reuses the open animator with no new field. (The earlier "translate `dragOffsetPx`" sketch was dropped: translating within a non-collapsing slot reads as a broken gap.)

Both paths track linearly while dragging (the raw fraction overrides the eased animator value mid-drag) and hand back to the eased curve on release, so the surface follows the pointer without easing lag but the settle/dismiss animates naturally.
