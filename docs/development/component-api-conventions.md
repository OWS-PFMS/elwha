...
|---|---|---|
| A **discrete named state** — selected, checked, expanded, collapsed, the active item | `firePropertyChange(PROPERTY_X, old, new)`, observed with `addPropertyChangeListener(PROPERTY_X, l)` | `ElwhaButton`, `ElwhaIconButton`, `ElwhaCheckbox`, `ElwhaBadge`, `ElwhaButtonSelectionGroup`, `ElwhaCard`, `ElwhaNavigationRail`, `ElwhaSwitch`, `ElwhaRadioButton`, `ElwhaRadioGroup`, `ElwhaTabs` |
...
**And "the pair is symmetric" is not an exemption.** #725 left `ElwhaCard` alone on the reading that its `addSelectionChangeListener` / `removeSelectionChangeListener` was complete and therefore not the defect being retired — but symmetry was never the rule, inheritance was, and the card is a `JComponent`. [#732](https://github.com/OWS-PFMS/elwha/issues/732) converted it, both pairs: the card held **two** private `PropertyChangeSupport` instances, one for selection and one for expansion, and each fired its property a second time immediately after the inherited `firePropertyChange` had already fired it. A private channel next to the inherited one is the drift the rule is about whether or not it comes with its `remove` — one of the two channels is invisible from the other, and a consumer's choice of subscription decided which library code they could interoperate with. The card now publishes `PROPERTY_SELECTED` and `PROPERTY_COLLAPSED` and nothing else, and `ElwhaCardChevron` / `ElwhaCardExpandLink` subscribe the same way a consumer does.
