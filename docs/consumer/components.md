...
| Icons | [`MaterialIcons`](https://ows-pfms.github.io/elwha/com/owspfm/elwha/icons/MaterialIcons.html) | Lookups for the bundled Material Symbols (Rounded, weight 400, fill 0, 20 px optical size), themed through a shared foreground filter. Roughly 65 named glyphs, each with a sized overload, plus `get(name)` / `pair(name)` for by-name lookup and the outline/fill toggle idiom, and `symbol(name)` — the typed handle the icon overloads of `ElwhaTab.of(...)` and `ElwhaNavRailDestination.of(...)` take. | Foundations → Icons |
...
| Drag cursors | [`list.ElwhaCursors`](https://ows-pfms.github.io/elwha/com/owspfm/elwha/list/ElwhaCursors.html) | `grab()` and `grabbing()` — the open-hand and closed-fist pointers AWT does not ship, as plain `java.awt.Cursor` values. `ElwhaItemList` wears them for drag-reorder; they are published so your own draggable surfaces read the same way. Both degrade to `MOVE_CURSOR` where custom cursors are unavailable, so no availability branch is needed. | — |
