...
A Swing component library built on [FlatLaf](https://www.formdev.com/flatlaf/) — `FlatCard`, `FlatPill`, and related primitives.
...
| **`FlatPill`** | A compact pill / chip primitive: text + optional leading icon + optional trailing action button, themeable via UIManager keys, auto-contrast foreground. |
| **`FlatPillList<T>`** | A list of `FlatPill` items with selection (`NONE` / `SINGLE` / `SINGLE_MANDATORY` / `MULTIPLE`), drag-to-reorder, pinned-partition + anchored modes, icon affordances. |
| **`FlatList<T>`** | The shared cross-cutting contract implemented by both `FlatCardList` and `FlatPillList` — orientation, gap, padding, empty / loading state, filter, sort. |
...
public class Demo {
  public static void main(String[] args) {
    SwingUtilities.invokeLater(() -> {
      ...
      // A simple pill
      FlatPill pill = new FlatPill("Tag");
      frame.add(pill);
...
- `com.owspfm.ui.components.pill.FlatPillPlayground`

Run them directly from a checkout: `mvn compile exec:java -Dexec.mainClass="com.owspfm.ui.components.pill.FlatPillPlayground"`.

## Theming

Every component reads theme defaults via `UIManager` under a component-specific namespace (`FlatPill.*`, etc.). Override at app startup before any component is created:

```java
UIManager.put("FlatPill.arc", 20);
UIManager.put("FlatPill.padding", new Insets(4, 12, 4, 12));
UIManager.put("FlatPill.background", new Color(0xEBF5FF));
```
...
- Two breaking-change epics are queued post-extraction:
  - `FlatPill` → `FlatChip` rename, aligning with Material's chip taxonomy
