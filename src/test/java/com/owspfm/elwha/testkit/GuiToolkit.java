 ...
 * </ul>
 *
 * <p>The condition doubles as an {@link AfterTestExecutionCallback}: any gui-tagged test failure
 * dumps the whole screen into {@code target/surefire-reports}, so the CI artifact carries visual
 * evidence. It must be this hook and not {@code TestWatcher} — the watcher runs after
 * {@code @AfterEach} has disposed the frame, so every capture came back an empty black screen; this
 * callback runs before teardown, while the window under test is still mapped. The per-class dump
 * helpers only fired on zero-delivery inside a step; the failure that motivated the widening burned
 * a full {@code WaitFor} deadline after every step "succeeded" and left no evidence at all (#585).
 *
 * @author Charles Bryan
 * @version v0.5.0
 * @since v0.5.0
 */
public final class GuiToolkit implements ExecutionCondition, AfterTestExecutionCallback {
