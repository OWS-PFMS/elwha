 ...
 *
 * <p><strong>Mandatory mode.</strong> A non-mandatory group ({@link #ElwhaButtonSelectionGroup()})
 * lets the user deselect the active button by clicking it again, leaving the group with no
 * selection — matches Swing's {@code ButtonGroup} default. A mandatory group ({@link
 * #setMandatory(boolean) setMandatory(true)}) refuses the deselect — clicking the
 * currently-selected mandatory button is a no-op (the group re-asserts the selection inside its own
 * listener). Calling {@link #setSelected(ElwhaButton) setSelected(null)} on a mandatory group
 ...
 *
 * <pre>{@code
 * ElwhaButtonSelectionGroup viewMode = new ElwhaButtonSelectionGroup()
 *     .setMandatory(true)
 *     .add(listButton)
 *     .add(gridButton)
 *     .add(compactButton);
 ...
 * @since v0.5.0
 */
public final class ElwhaButtonSelectionGroup {
   ...
   * @since v0.5.0
   */
  public ElwhaButtonSelectionGroup() {
   ...
   * @since v0.5.0
   */
  public ElwhaButtonSelectionGroup add(final ElwhaButton button) {
    if (button == null) {
      throw new NullPointerException("button");
    }
    if (button.getInteractionMode() != ButtonInteractionMode.SELECTABLE) {
      throw new IllegalArgumentException(
          "ElwhaButtonSelectionGroup requires SELECTABLE buttons; got "
   ...
   * @since v0.5.0
   */
  public ElwhaButtonSelectionGroup remove(final ElwhaButton button) {
   ...
   * @since v0.5.0
   */
  public ElwhaButtonSelectionGroup setSelected(final ElwhaButton button) {
    if (button == null) {
      if (mandatory) {
        throw new IllegalStateException(
            "Cannot clear selection on a mandatory ElwhaButtonSelectionGroup — exactly one button"
    ...
    if (!buttons.contains(button)) {
      throw new IllegalArgumentException(
          "button is not a member of this ElwhaButtonSelectionGroup");
    }
    adjusting = true;
    try {
      for (ElwhaButton b : buttons) {
   ...
   * @since v0.5.0
   */
  public ElwhaButtonSelectionGroup setMandatory(final boolean mandatory) {
