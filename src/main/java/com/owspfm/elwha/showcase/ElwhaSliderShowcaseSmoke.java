 ...
 * the standard + centered + range + size + inset-icon 12×5 state matrix (with the original nine
 * config rows still {@code XS} and the new rows scaling up / carrying an inset icon), and that the
...
public final class ElwhaSliderShowcaseSmoke {
  ...
  public static void main(final String[] args) {
    ...
    // 12 configs (3 standard + 3 centered + 3 range + Size M + Size XL + inset) × 5 states = 60.
    check("gallery is the 12×5 state matrix", countSliders(gallery) == 60);
    ...
    // The original 9 config rows stay XS (visually unchanged); only the 3 new rows scale up.
    check("the original 9 config rows are still XS", countSize(gallery, ElwhaSlider.Size.XS) == 45);
