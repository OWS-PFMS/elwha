...
/**
 * Interactive playground for {@link ElwhaChip} and {@link ElwhaChipList}.
 *
 ...
 *   <li><strong>Live list</strong>: a {@link ElwhaChipList} the user can switch between all four
 ...
 * <pre>
 *   mvn -q exec:java \
 *     -Dexec.mainClass=com.owspfm.elwha.chip.ElwhaChipPlayground
 * </pre>
 *
...
public final class ElwhaChipPlayground {
  ...
  public static void main(final String[] args) {
    SwingUtilities.invokeLater(
        () -> {
          FlatLightLaf.setup();
          new ElwhaChipPlayground().launch();
        });
  }
  ...
  private JPanel buildVariantGallery() {
    ...
    // MaterialIcons.DEFAULT_SIZE bump to 24 (M3 icon-button standard). Revisited in ElwhaChip V2.
    ...
    for (int c = 0; c < icons.length; c++) {
      ...
      final ElwhaChip chip = new ElwhaChip(iconName).setVariant(v);
   ...
   * because {@link ElwhaChipList#setMovementMode} clears the inactive side's predicate/action under
  ...
  private ElwhaChip buildLiveChip(final String item) {
    ...
    // UIManager.put("ElwhaChip.removeIcon", icon) is still honored.
    final javax.swing.Icon trashIcon =
        UIManager.get("ElwhaChip.removeIcon") instanceof javax.swing.Icon override
            ? override
  ...
  private JPanel buildLiveListControls() {
    ...
    anchorAff.addActionListener(
        e -> list.setAnchorAffordance((ElwhaChipList.IconAffordance) anchorAff.getSelectedItem()));
    p.add(anchorAff);
  ...
  private JPanel buildLafPanelWrapper() {
    ...
    addLafRow(grid, "Background", buildColorPicker(ElwhaChip.K_BACKGROUND));
    addLafRow(grid, "Foreground", buildColorPicker(ElwhaChip.K_FOREGROUND));
    addLafRow(grid, "Border color", buildColorPicker(ElwhaChip.K_BORDER_COLOR));
    addLafRow(grid, "Hover background", buildColorPicker(ElwhaChip.K_HOVER_BACKGROUND));
    addLafRow(grid, "Pressed background", buildColorPicker(ElwhaChip.K_PRESSED_BACKGROUND));
    addLafRow(grid, "Selected background", buildColorPicker(ElwhaChip.K_SELECTED_BACKGROUND));
    addLafRow(grid, "Selected border", buildColorPicker(ElwhaChip.K_SELECTED_BORDER_COLOR));
    addLafRow(grid, "Focus color", buildColorPicker(ElwhaChip.K_FOCUS_COLOR));
    addLafRow(grid, "Disabled background", buildColorPicker(ElwhaChip.K_DISABLED_BACKGROUND));
    addLafRow(grid, "Warm accent", buildColorPicker(ElwhaChip.K_WARM_ACCENT));

    final JButton reset =
        new JButton(
            new AbstractAction("Reset all ElwhaChip.* keys") {
              @Override
              public void actionPerformed(final ActionEvent e) {
                resetAllKeys();
              }
  ...
  private JSlider buildArcSlider() {
    ...
    // the visible cap (clampArc() in ElwhaChip bounds it by min(width, height) at paint time).
    final JSlider s = new JSlider(0, 60, 20);
    s.setMajorTickSpacing(20);
    s.setPaintTicks(true);
    s.addChangeListener(
        e -> {
          UIManager.put(ElwhaChip.K_ARC, s.getValue());
          repaintLive();
        });
    return s;
  }
  ...
  private JSlider buildPaddingSlider(final boolean horizontal) {
    ...
    s.addChangeListener(
        e -> {
          ...
          final Insets cur =
              UIManager.get(ElwhaChip.K_PADDING) instanceof Insets in
                  ? in
          ...
          UIManager.put(ElwhaChip.K_PADDING, next);
          ...
          // updateUI cascade, which causes ElwhaChip.updateUI() to call rebuildBorder() and
  ...
  private void resetAllKeys() {
    final String[] keys = {
      ElwhaChip.K_BACKGROUND,
      ElwhaChip.K_FOREGROUND,
      ElwhaChip.K_BORDER_COLOR,
      ElwhaChip.K_ARC,
      ElwhaChip.K_PADDING,
      ElwhaChip.K_HOVER_BACKGROUND,
      ElwhaChip.K_PRESSED_BACKGROUND,
      ElwhaChip.K_SELECTED_BACKGROUND,
      ElwhaChip.K_SELECTED_BORDER_COLOR,
      ElwhaChip.K_FOCUS_COLOR,
      ElwhaChip.K_DISABLED_BACKGROUND,
      ElwhaChip.K_WARM_ACCENT
    };
    for (String k : keys) {
