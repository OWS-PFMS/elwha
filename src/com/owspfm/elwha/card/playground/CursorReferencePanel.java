...
/**
 * Reference panel that lists every cursor available to {@link com.owspfm.elwha.card.list
 * .ElwhaCardList} alongside a hover-zone cell that activates that cursor, so callers can compare
 * them side-by-side.
 *
...
public final class CursorReferencePanel extends JPanel {
  ...
  private static Cursor loadCustomCursor(final String name) {
    try {
      final Class<?> clazz = Class.forName("com.owspfm.elwha.card.list.Cursors");
