...
A reusable, model-driven `Swing` list of `ElwhaCard`s. Drop-in replacement for the
...
standard Swing — and depends only on its sibling `com.owspfm.elwha.card.v1`
...
```
mvn -q exec:java \
  -Dexec.mainClass=com.owspfm.elwha.card.v1.list.ElwhaCardListDemo
```

The full playground (interactive controls, snippet pane, theme switcher) lives
in `com.owspfm.elwha.card.v1.playground.ElwhaCardPlayground` under the
**ElwhaCardList** tab.
...
1. Move the `com/owspfm/elwha/card/list` directory (and its sibling
