...
/**
 * The 6-step spacing scale of the FlatComp token vocabulary — a 4px-based ladder for paddings,
 * gaps, and insets.
 *
 * <p>Material 3 has no formal spacing <em>token</em> scale (it uses an informal 4px/8px grid), so
 * this is FlatComp's own ladder on that grid. Each step resolves at paint time from {@link
 * UIManager} under the key {@code FlatComp.space.<key>}, which {@link FlatCompTheme#install}
 * writes. As with {@link ShapeScale}, the v1 values are fixed but still resolved — see {@code
 * flatcomp-theme-install-api.md} §1.4.
 *
...
public enum SpaceScale {
  ...
  private static final String KEY_PREFIX = "FlatComp.space.";
  ...
  /**
   * Returns the compiled-in v1 default length for this step — the value {@link FlatCompTheme}
