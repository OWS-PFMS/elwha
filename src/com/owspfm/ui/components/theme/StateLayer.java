...
/**
 * The 5 interaction-state layers of the FlatComp token vocabulary, expressed the Material 3 way —
 ...
 * FlatComp.state.<key>}, which {@link FlatCompTheme#install} writes. The {@link #over} methods
 ...
 *
 * <p>FlatComp's own components apply these overlays live at paint time. The raw-Swing bridge
 * instead bakes them into FlatLaf's discrete {@code *hoverBackground} / {@code *pressedBackground}
 * keys at install time — see {@code flatcomp-theme-install-api.md} §5.
 *
...
public enum StateLayer {
  ...
  /**
   * Selected overlay (12%) — a FlatComp-specific layer applied uniformly across chip and card
   * lists. M3 models selection as a container-color swap rather than an overlay; FlatComp keeps one
  ...
  private static final String KEY_PREFIX = "FlatComp.state.";
  ...
  /**
   * Returns the compiled-in M3 default opacity for this layer — the value {@link FlatCompTheme}
