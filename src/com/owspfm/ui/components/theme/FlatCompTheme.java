 ...
 * no separate "switch" API, switching <em>is</em> re-installing. {@code FlatCompTheme} is a static
 * facade rather than an instantiable object because Swing has exactly one look-and-feel per JVM, so
 * a global static API matches physical reality (see {@code flatcomp-theme-install-api.md} §8, Q5).
 *
 * <pre>{@code
 * // Startup
 ...
 * FlatCompTheme.install(FlatCompTheme.current().withMode(Mode.DARK));
 * }</pre>
 *
 ...
 * write the {@code FlatComp.*} keys, write the FlatLaf-native keys, compute-and-bake the
 ...
 * @since v0.1.0
 */
public final class FlatCompTheme {

  private static volatile Config current;

  private FlatCompTheme() {}
